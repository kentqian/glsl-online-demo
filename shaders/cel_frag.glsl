...
void main() {
  ...
  float Cel_I_diffuse = floor(I_diffuse * 3.0)/ 3.0;

  vec4 eye_vec = normalize(vec4(-vertPos,0.0));
  vec4 H = normalize(L_dir + eye_vec);
  float intSpec = max(dot(N,H),0.0);
  float Cel_I_specular = 0.0;
  if (dot(N,L_dir) > 0.0){
  	float I_specular = pow(intSpec, shininessVal);
 	Cel_I_specular = floor(I_specular * 3.0)/ 3.0;
  }

  gl_FragColor = Ka * vec4(ambientColor, 1.0) + Cel_I_diffuse * Kd * vec4(diffuseColor, 1.0) + Ks * vec4(specularColor, 1.0) * Cel_I_specular;
}